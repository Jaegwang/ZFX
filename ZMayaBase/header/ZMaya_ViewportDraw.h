//----------------------//
// ZMaya_ViewportDraw.h //
//-------------------------------------------------------//
// author: Wanho Choi @ Dexter Studios                   //
// last update: 2016.12.30                               //
//-------------------------------------------------------//

#ifndef _ZMaya_ViewportDraw_h_
#define _ZMaya_ViewportDraw_h_
...
class ZDrawData : public MUserData
{
	public:
...
template <class T>
class ZDrawOverride : public MHWRender::MPxDrawOverride
{
	protected:
...
template <class T>
ZDrawOverride<T>::ZDrawOverride( const MObject& dgNodeObj )
: MHWRender::MPxDrawOverride( dgNodeObj, ZDrawOverride::draw )
{
...
ZDrawOverride<T>::Creator( const MObject& dgNodeObj )
{
	return new ZDrawOverride( dgNodeObj );
}

template <class T>
MHWRender::DrawAPI
ZDrawOverride<T>::supportedDrawAPIs() const
{
...
template <class T>
bool
ZDrawOverride<T>::isBounded( const MDagPath& dgNodePath, const MDagPath& cameraPath ) const
{
	return true;
}
...
template <class T>
MBoundingBox
ZDrawOverride<T>::boundingBox( const MDagPath& dgNodePath, const MDagPath& cameraPath ) const
{
...
template <class T>
MUserData*
ZDrawOverride<T>::prepareForDraw( const MDagPath& dgNodePath, const MDagPath& cameraPath, const MHWRender::MFrameContext& frameContext, MUserData* oldData )
{
	...
	ZDrawData* drawData = dynamic_cast<ZDrawData*>( oldData );
	if( !drawData ) { drawData = new ZDrawData(); }
...
template <class T>
void
ZDrawOverride<T>::draw( const MHWRender::MDrawContext& context, const MUserData* data )
{
	...
	const ZDrawData* drawDataPtr = dynamic_cast<const ZDrawData*>( data );
	if( !stateMgr || !drawDataPtr ) { return; }
	const ZDrawData& drawData = *drawDataPtr;
